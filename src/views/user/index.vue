<template>
  <div class="user-container">
        ...
        <div class="user-info-block">
          <div class="avatar">
            <a-upload
              ...
              v-if="user.id === getUid() && userStore.user.id === getUid()"
            >
              <template #upload-button>
                <div
                  style="
                    ...
                    text-align: center;
                  "
                  v-if="user.avatar === null || user.avatar == undefined || user.avatar === ''"
                >
                  <div>
                    <span style="color: #3370ff; line-height: 80px">点击上传图片</span>
                  </div>
                </div>
                <img
            ...
            <div class="h-title">
              ...
              <template v-if="user.role != null">
                <a-tag
                  ...
                  v-if="user.role != null && user.role.ident === 'admin'"
                  :color="'magenta'"
                  bordered
                  >{{ user.role.roleName }}</a-tag
                >
    ...
    <div class="user-bottom-nav">
      <a-menu
        mode="horizontal"
        :default-selected-keys="['articleList']"
        :selected-keys="[$route.name === 'userDefault' ? 'articleList' : $route.name]"
...
import { updateUser, getUser, logout } from '@/api/user'
...
const background = computed(() => {
  return user.value.background != null && user.value.background && user.value.background != ''
    ? setting.baseUrl + user.value.background
    : setting.failImg
})
...
function customRequest(option) {
  ...
  upload(formData)
    .then((res) => {
      updateUser({ avatar: res.msg })
        .then((user.value.avatar = userStore.user.avatar = res.msg))
        .catch((e) => e)
    })
    .catch((r) => r)
}
