<template>
    <div>
        ...
        <div class="setting-collapse">
            <a-collapse accordion @change="collapseHandler" destroy-on-hide>
                <a-collapse-item v-for="favorites in favoritesList" :key="favorites.id" :style="customStyle">
                    <template #header>
                        <div>
                            {{ favorites.title }} 
                            ...
                            <a-trigger trigger="click" :unmount-on-close="false" @click.stop>
                                ...
                                <template #content>
                                    <div class="demo-basic">
                                        ...
                                        <a-doption @click.stop="deleteFavoritesHandler(favorites.id)">删除收藏夹</a-doption>
                                    </div>
                                </template>
                            </a-trigger>
                        </div>
                    ...
                    <div v-else>
                        ...
                        <EditArticleList v-else @del="removeHandler" :data-source="dataSource" :needEdit="false">
                        </EditArticleList>
                    </div>
                </a-collapse-item>
            </a-collapse>
    ...
    <a-modal v-model:visible="visible" @ok="handleOk" @cancel="handleCancel"  
        :ok-button-props="{disabled: currentFavoritesEdit.name === null || currentFavoritesEdit.name === ''}">
        <template #title>
            修改收藏夹
        </template>
    ...
    <!-- dialog -->
    <a-modal v-model:visible="addVisible" @ok="HandleAddOk" @cancel="addVisible = false" 
        :ok-button-props="{disabled: favoritesForm.name === null || favoritesForm.name === undefined || favoritesForm.name === ''}">
        <template #title>
            新增收藏夹
        </template>
        <div class="dialog-form">
