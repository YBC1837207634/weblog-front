  ...
  <div>
    <a-list :bordered="false" :size="'large'">
      <a-list-item v-for="(item, index) in data" :key="item.id" class="list-demo-item">
        <a-list-item-meta
          :title="item.nickname"
          @click="$router.push({ name: 'userDefault', params: { id: item.id } })"
          :description="item.description"
        >
          <template #avatar>
            <a-avatar shape="square">
              <img alt="avatar" :src="setting.baseUrl + item.avatar" v-if="item.avatar" />
              <span v-else>U</span>
            </a-avatar>
          </template>
        </a-list-item-meta>
...
function relationHandler(id, index) {
  relation({ goalId: id, act: data.value[index].relationOk ? 0 : 1 }).then((res) => {
