<template>
  <div class="article-container">
    <div class="left-info">
      <div class="content">
        ...
        <div v-if="articleContent.author">
          <SuspendedPanel
            v-if="articleContent.author.id !== getUid()"
      ...
      <div class="comment">
        ...
        <div class="comment-text">
          ...
          />
          <a-button type="primary" size="large" @click="commentHandler" v-if="userStore.isLogin"
            >发送</a-button
          >
        </div>
        <a-comment
        ...
        >
          <template #avatar>
            <a-trigger position="top" scroll-to-close auto-fit-position unmount-on-close>
              <a-avatar>
                <img alt="avatar" :src="getAvatar(comment.commentator.avatar)" />
              </a-avatar>
              <template #content>
          ...
          <!-- 子评论 -->
          <a-comment
            v-for="(subComment, subIndex) in comment.subComments.slice(
              currentComment === comment.id ? start : 0,
              currentComment === comment.id ? end : 2
            )"
            v-if="comment.subComments != null && comment.subComments.length > 0"
            :key="subComment.id"
            :author="
              subComment.target == null
                ? subComment.commentator.nickname
                : subComment.commentator.nickname + ' 回复 ' + subComment.target.nickname
            "
          ...
          >
            <template #avatar>
              <a-trigger position="top" scroll-to-close auto-fit-position unmount-on-close>
                <a-avatar>
                  <img alt="avatar" :src="getAvatar(subComment.commentator.avatar)" />
                </a-avatar>
                <template #content>
                  <UserCard title="用户信息" :user-info="subComment.commentator"></UserCard>
                </template>
              </a-trigger>
            </template>
            <template #actions>
            ...
            <ReplyInput
              ...
              :scope="{
                ...
                toId: subComment.commentator.id,
                parentId: comment.id,
                commentType: '3'
              }"
              :avatar="getAvatar(userStore.getUser.avatar)"
          ...
          <div
            v-if="
              ...
              comment.subComments instanceof Array &&
              comment.subComments.length > 3
            "
            class="expand-comment"
          >
            共{{ comment.subComments.length }}条消息，<span
              class="click-look"
        ...
        <div class="more-comment" v-if="articleContent.commentCounts > 5 && moreCommentBtn">
          <div class="text" @click="getMoreComment">
            查看全部 {{ articleContent.commentCounts }} 条评论
          </div>
        </div>
      </div>
    ...
    <div class="right-card">
      <!-- 角色卡片 -->
      <UserCard
        v-if="articleContent.author"
        title="作者信息"
        :userInfo="articleContent.author"
      ></UserCard>
      <a-card title="文章信息">
...
import { getHostTagList } from '@/api/tag'
...
onMounted(() => {
  ...
  articleRank().then((res) => {
    articleRankList.value = res.data.filter((item) => item.id !== route.params.id)
  })
})

function getAvatar(url) {
  return url != null && url != '' ? setting.baseUrl + url : setting.failImg
}
...
function getMoreComment() {
  params.pageSize = articleContent.value.commentCounts
  getList()
  moreCommentBtn.value = false
}
...
const onTrampleChange = (data) => {
  if (data.two === -1) {
    commentList.value[data.one].trample = !commentList.value[data.one].trample
  } else {
