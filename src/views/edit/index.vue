  ...
  <div class="edit-content">
    <div class="title">
      ...
      <div class="btn">
        ...
        <a-dropdown-button v-else style="margin-left: auto" @click="$router.push('/')">
          主页
          <template #icon>
            <icon-down />
          </template>
    ...
    <div class="drawer">
      <a-form ref="formRef" :size="'large'" :model="form" @submit-success="handleSubmitSuccess">
        <a-form-item
          ...
          label="类别"
          :rules="[
            { true: true, message: '至少选择一个' },
            { empty: false, message: '至少选择一个' },
        ...
        >
          <a-select
            ...
            placeholder="选择类别"
            allow-clear
            @dropdown-reach-bottom="dropdownHandler"
          >
            <a-option v-for="tag in category" :key="tag.name + '5'" :value="tag.id">{{
              tag.tagName
            }}</a-option>
          </a-select>
        </a-form-item>
        ...
        >
          <a-select
            ...
            allow-search
          >
            <a-option v-for="tag in tags" :key="tag.name + '2'" :value="tag.id">{{
              tag.tagName
            }}</a-option>
          </a-select>
        </a-form-item>
        <a-form-item label="封面" field="img">
          <a-upload show-preview-button :show-file-list="false" :custom-request="customRequest">
            <template #upload-button>
              ...
              <a-image v-else width="224" :src="setting.baseUrl + imgUrl" />
            </template>
          </a-upload>
        </a-form-item>
...
import MarkLoading from '@/components/MarkLoading.vue'
