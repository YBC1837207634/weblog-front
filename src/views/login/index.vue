    ...
    <div class="main">
        <div id="page" class="site">
            <div class="container">
                <div class="login">
                    <div class="main">
                        <a-form v-if="!reg" class="user-form" ref="formRef" size="middle" :layout="'vertical'" :model="form" @submit-success="handleSubmitSuccess" :label-align="'left'">
                            <a-form-item field="username" label="帐号"
                                :rules="[{ required: true, message: '帐号不可为空' }, { minLength: 2, message: '帐号不可为空且不小于2个字符' }]"
                                :validate-trigger="['change', 'input']">
                                <input type="text" v-model="form.username"  placeholder="帐号">
                            </a-form-item>
                            <a-form-item field="password" label="密码"
                                :rules="[{ required: true, message: '密码不可为空' }, { max: 30, message: '密码不可为空' }]">
                        ...
                        <a-form v-else class="user-form" ref="formRef" size="middle" :layout="'vertical'" :model="form" @submit-success="registerHandler" :label-align="'left'">
                            <a-form-item field="username" label="帐号"
                                :rules="[{ required: true, message: '帐号不可为空' }, { minLength: 2, message: '帐号不可为空且不小于2个字符' }]"
                                :validate-trigger="['change', 'input']">
                                <input type="text" v-model="form.username"  placeholder="帐号">
                            ...
                            <a-form-item field="password" label="密码"
                                :rules="[{ required: true, message: '密码不可为空' }, { max: 64 , message: '密码大于64字符' }]"
                                :validate-trigger="['change', 'input']">
                                <input type="password" v-model="form.password" autocomplete placeholder="密码"> 
                            </a-form-item>
                            <a-form-item field="code" label="动态码"
                                :rules="[{ required: true, message: '动态码不可为空' }, { max: 6, message: '动态码不可为空' }]">
