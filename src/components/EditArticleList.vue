<template>
   <div>
    <div class="btn">
        <a-popconfirm v-if="checked.length > 0" :content="`确认删除${checked.length}条数据`" type="warning" @ok="batchDelHandler">
            <a-button type="primary" size="large" long><template #icon>
            <icon-delete style="font-size: 14px;"/>
            </template>确认删除：{{ checked.length }}</a-button>
        </a-popconfirm>
