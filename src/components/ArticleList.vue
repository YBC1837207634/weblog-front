  ...
  <a-list class="list-demo-action-layout" :bordered="false" :data="dataSource">
    <template #item="{ item }">
      <a-list-item class="list-demo-item" action-layout="vertical">
        <a-list-item-meta @click="clickHandler(item.id)" :title="item.title">
        ...
        <template #extra v-if="item.img != null && item.img != ''">
          <div class="image-area">
            <a-image width="170" :src="setting.baseUrl + item.img" />
          </div>
          <div class="image-area-phone">
            <a-image width="150" :src="setting.baseUrl + item.img" />
          </div>
        </template>
      </a-list-item>
    </template>
